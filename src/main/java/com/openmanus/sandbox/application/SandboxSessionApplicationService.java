package com.openmanus.sandbox.application;
 ...
 */
@Slf4j
public class SandboxSessionApplicationService {

    private final SandboxRuntimePort sandboxRuntimePort;
    private final SandboxWorkspacePort sandboxWorkspacePort;
    ...
    public SandboxSessionApplicationService(
            SandboxRuntimePort sandboxRuntimePort,
            SandboxWorkspacePort sandboxWorkspacePort) {
        this.sandboxRuntimePort = sandboxRuntimePort;
        this.sandboxWorkspacePort = sandboxWorkspacePort;
        log.info("SandboxSessionApplicationService 初始化完成");
    }
    ...
    public Optional<SessionSandboxInfo> getSandboxInfo(String sessionId) {
        ...
        try {
            SessionSandboxInfo refreshedInfo = sandboxRuntimePort.refreshSandboxInfo(sessionId, info);
    ...
    public synchronized SessionSandboxInfo getOrCreateSandbox(String sessionId) {
        ...
        try {
            ...
            SessionSandboxInfo sandboxInfo = sandboxRuntimePort.createSandbox(sessionId);
            if (sandboxInfo == null) {
    ...
    public String getWorkspaceRoot(String sessionId) {
        return sandboxWorkspacePort.getWorkspaceRoot(sessionId);
    }

    /**
     * 将用户路径解析到容器工作区内，阻止路径越界。
     */
    public String resolveWorkspacePath(String sessionId, String userPath) {
        return sandboxWorkspacePort.resolveWorkspacePath(sessionId, userPath);
    }
    ...
    public SandboxCommandResult executeCommand(String sessionId, String command, String cwd, int timeoutSeconds) {
        return sandboxRuntimePort.executeCommand(sessionId, command, cwd, timeoutSeconds);
    }
    ...
    public SandboxCommandResult openBrowserUrl(String sessionId, String url) {
        return sandboxRuntimePort.openBrowserUrl(sessionId, url);
    }
    ...
    public SandboxCommandResult executePython(String sessionId, String script, int timeoutSeconds) {
        return sandboxRuntimePort.executePython(sessionId, script, timeoutSeconds);
    }
    ...
    public String readTextFile(String sessionId, String path) {
        return sandboxRuntimePort.readTextFile(sessionId, path);
    }
    ...
    public void writeTextFile(String sessionId, String path, String content) {
        sandboxRuntimePort.writeTextFile(sessionId, path, content);
    }
    ...
    public void destroySandbox(String sessionId) {
        ...
        if (info != null) {
            try {
                ...
                sandboxRuntimePort.destroySandbox(sessionId);
            } catch (Exception e) {
