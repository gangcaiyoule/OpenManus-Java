...
public final class ContextBudgetPolicy {
    ...
    private final ToIntFunction<AiChatMessage> modelContextTokenCounter;

    public ContextBudgetPolicy(int maxHistoryMessages, int maxTotalMessages, int maxApproxTokens) {
        this(maxHistoryMessages, maxTotalMessages, maxApproxTokens, ModelContextTokenCounter.approx());
    }

    public ContextBudgetPolicy(int maxHistoryMessages,
                               int maxTotalMessages,
                               int maxApproxTokens,
                               ToIntFunction<AiChatMessage> modelContextTokenCounter) {
        ...
        this.modelContextTokenCounter = modelContextTokenCounter == null
                ? ModelContextTokenCounter.approx()
    ...
    public List<AiChatMessage> applyApproxTokenBudget(List<AiChatMessage> messages, AiChatMessage currentUserMessage) {
        ...
        List<AiChatMessage> budgetedRuntimeMessages = applyTokenBudget(
                safeMessages,
                currentUserMessage,
                maxApproxTokens,
                modelContextTokenCounter
    ...
    private static List<AiChatMessage> applyTokenBudget(List<AiChatMessage> safeMessages,
                                                        AiChatMessage currentUserMessage,
                                                        int maxTokens,
                                                        ToIntFunction<AiChatMessage> tokenCounter) {
        if (safeMessages.isEmpty() || maxTokens <= 0) {
            return safeMessages;
        }
        if (tokenCounter == null) {
            tokenCounter = ModelContextTokenCounter.approx();
        }
        ...
        int fixedTokens = estimateTokens(fixed, tokenCounter);
        ...
        for (int i = safeMessages.size() - 1; i >= 0; i--) {
            ...
            int candidateTokens = tokenCounter.applyAsInt(candidate);
    ...
    private static List<AiChatMessage> minimalCriticalSet(AiChatMessage currentUserMessage,
        ...
        for (AiChatMessage message : prioritized) {
            int tokens = tokenCounter.applyAsInt(message);
    ...
    private static int estimateTokens(List<AiChatMessage> messages, ToIntFunction<AiChatMessage> tokenCounter) {
        if (messages == null || messages.isEmpty()) {
            return 0;
        }
        ToIntFunction<AiChatMessage> safeCounter = tokenCounter == null
                ? ModelContextTokenCounter.approx()
                : tokenCounter;
        int total = 0;
        for (AiChatMessage message : messages) {
            total += safeCounter.applyAsInt(message);
        }
        return total;
    }
