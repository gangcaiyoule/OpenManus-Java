...
public class PythonExecutionTool {
    ...
    public PythonExecutionTool(AiCodeSandbox sandbox, SandboxPathResolver sandboxPathResolver) {
    ...
    public String executePythonFile(@AiParam("Python文件路径") String filePath) {
        ...
        try {
            ...
            String code = sandboxPathResolver.readTextFile(path.toString());
            AiCodeExecutionResult result = sandbox.executePython(code, DEFAULT_TIMEOUT_SECONDS);
            return formatExecutionResult(result);
            ...
            if (e.getMessage() != null && e.getMessage().contains("读取沙箱文件失败")) {
